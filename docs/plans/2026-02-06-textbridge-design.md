...
> BLE 모드: ZMK이 BLE 라디오를 HID 광고에 사용 중 (단일 광고 제약) + Fn+1~3이 프로필 페어링에 할당 → 미지원.
...
```
① 최초 페어링  Fn+1 (3초 홀드) → TextBridge 페어링 모드 → 폰과 본딩 (1회)
                                                    ↓
...
- 최초 1회: Fn+1을 3초 이상 홀드하여 페어링 모드 진입 → 폰과 본딩
- 이후: `CONFIG_BT_FILTER_ACCEPT_LIST=y` → 본딩된 폰만 자동 연결 허용
- 타인 연결 불가 (본딩되지 않은 기기 차단)
- 재연결: Fn+1 탭 (< 3초) → 본딩된 폰에 재연결 시도
- 재페어링: Fn+1 홀드 (≥ 3초) → 새 기기 등록 (기존 본딩 교체)
...
- TextBridge 광고 데이터에 커스텀 UUID만 포함 (HID UUID 미포함, Appearance=Unknown)
- 광고 단계에서 폰이 키보드로 인식하지 않음
- **주의:** `BT_GATT_SERVICE_DEFINE`은 정적 등록이므로, 연결 후 GATT Service Discovery에서 ZMK HOG(0x1812) 서비스가 노출될 수 있음. iOS/Android가 자동으로 HID 키보드로 전환하는지 Phase 2에서 실제 테스트 필요. 문제 시 앱에서 TextBridge 서비스만 사용하도록 제한.
...
BLE MTU 기본 23bytes → ATT 헤더 3bytes → ATT 페이로드 20bytes → 앱 헤더 3bytes → 키코드 17bytes → 8개/청크
MTU 협상 후 최대 244bytes → ATT 페이로드 241bytes → 앱 헤더 3bytes → 키코드 238bytes → 119개/청크
```
...
ACK(Notify) 손실 시 폰이 동일 청크를 재전송할 수 있다.
키보드는 마지막 처리한 시퀀스 번호를 저장하여 중복을 감지한다:

- 수신 시퀀스 == 마지막 처리 시퀀스 → HID 주입 건너뜀, ACK만 재전송
- 수신 시퀀스 != 마지막 처리 시퀀스 → 정상 처리 후 ACK
