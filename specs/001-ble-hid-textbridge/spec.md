...
2. **Given** 대상 OS가 macOS로 설정된 상태,
   **When** 사용자가 `가나다`를 전송하면,
   **Then** Ctrl+Space(0x2C, modifier 0x01)로 한영 전환 후 동일한 결과가 나온다.
3. **Given** BLE 연결 상태,
...
1. **Given** 설정 화면,
   ...
   **Then** 한글 전송 시 사용되는 한영 전환키가 0x90에서 Ctrl+Space(0x2C, mod=0x01)로 변경된다.
2. **Given** 설정 화면,
...
- **FR-014**: 한글-영문 전환 시 대상 OS에 맞는 한영 전환키를 자동으로
  삽입해야 한다 (Windows: 0x90/Lang1, macOS: Ctrl+Space 0x2C mod=0x01).
  토글키는 반드시 **단독 청크(1개 키코드)로 분리**하여 전송하고,
  키보드의 ACK를 수신한 후에 다음 청크를 전송해야 한다.
  이는 OS의 입력기 전환 완료를 보장하기 위한 동기식 확인 메커니즘이다.
...
- **FR-016**: 앱은 대상 OS(Windows/macOS) 선택을 제공해야 하며,
  ...
  (Windows: 0x90/Lang1, macOS: Ctrl+Space 0x2C mod=0x01).
...
- Q: macOS 한영 전환키를 Right GUI(0xE7)에서 Ctrl+Space(0x2C, mod=0x01)로 변경할 것인가? → A: Ctrl+Space로 변경. Right GUI는 HID 인젝션 시 macOS에서 작동하지 않음이 실제 하드웨어 테스트로 확인됨.
- Q: SC-003 대용량 연속 전송 성공 기준 횟수? → A: 5회에서 3회로 하향. 현재 3회째 ACK 타임아웃 발생하므로 근본 원인 해결 후 점진적 개선.
- Q: 구현 완료 후 스펙 상태를 변경할 것인가? → A: "Active"로 변경. 구현 완료되었으나 Flutter macOS 토글키 수정, 스트레스 테스트 안정화 등 미해결 이슈 잔존.
- Q: 한영 전환키를 다른 키코드와 같은 청크에 포함해도 되는가? → A: 안 된다. 토글키는 반드시 단독 청크(1개 키코드)로 분리하여 전송하고, 키보드의 ACK(100ms 딜레이 포함)를 수신한 후 다음 청크를 전송한다. 이는 OS 입력기 전환 완료를 ACK 왕복으로 확인하는 동기식 메커니즘이다.
